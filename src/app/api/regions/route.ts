...
export async function GET() {
  try {
    ...
    // Парсим CSV с разделителем ';' (или ',' - проверьте ваш файл)
    const records = parse(fileContent, {
      columns: ['code', 'parentCode', 'name', 'postal_code'],
      skip_empty_lines: true,
      delimiter: ';', // или ',' - в зависимости от формата вашего CSV
      relax_quotes: true,
    });

    // Фильтруем только регионы (parentCode пустой и code длиной 2 символа)
    const regions = records
      .filter((record: any) => 
        (!record.parentCode || record.parentCode === '') && 
        record.code.length === 2
      )
